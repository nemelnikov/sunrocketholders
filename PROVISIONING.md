# How to provision a new environment for sunrocketholders
...
* Enter domains to secure (for example): `sun.melnikov.family`
* Enter webroot path: `/home/deployer/apps/sunrocketholders/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/sun.melnikov.family/fullchain.pem /etc/ssl/sunrocketholders.crt
ln -s /etc/letsencrypt/live/sun.melnikov.family/privkey.pem /etc/ssl/sunrocketholders.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/sunrocketholders/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
52 0,12 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/sunrocketholders/shared/public/
```
